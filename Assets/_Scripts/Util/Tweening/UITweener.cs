...
public abstract class Tweener : MonoBehaviour
{
	...
	[HideInInspector]
	public Easings.Function method = Easings.Function.Linear;
	...
	public void SetOnFinishedOneShot(EventDelegate.Callback del){
		EventDelegate eventDel=new EventDelegate(del);
		eventDel.oneShot = true;
		SetOnFinished (eventDel);
	}
	...
	public void Sample (float factor, bool isFinished)
	{
		...
		val = Easings.Interpolate(val, method);
	...
	public void ResetFactorReverse(){
		mStarted=false;
		mFactor=1;
		mAmountPerDelta = -Mathf.Abs((duration > 0f) ? 1f / duration : 1000f);
		OnUpdate(1,false);
	...
	static public T Begin<T> (GameObject go, float duration, float delay = 0f) where T : Tweener
	{
